module Robosolver
  module Solvers
    class Djikstra
      # From http://rosettacode.org/wiki/Dijkstra%27s_algorithm#Ruby
      class Graph
      ...
      def solve
        path, _dist = board_graph.shortest_path(start.name, target.name)
        path
      end
      ...
      def build_board_graph
        edges = board.cells.flatten.map do |cell|
          cell.neighbors.map do |nx, ny|
            [cell.name, board.at(nx, ny).name, 1]
          end
        end.flatten(1).uniq
